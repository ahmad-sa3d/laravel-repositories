...
abstract class BaseRepository implements
    ...
    public function update($id_or_object, array $attributes) {
        ...
        // Updating
        if (!$this->skip_mutator && $this->mutator) {
            $this->mutator->update($object, $attributes);
        } else {
     ...
     * @return RepositoryContract Model Name
     */
     ...
     *
     * @param  Builder $context EloquentBuilder Or Model Object
    ...
    protected function makeModel()
    {
        ...
        // Create New Model Query Builder
        $this->resetBuilder();
    }
     ...
     * @param  boolean $single_object get only the first object or all collection
     * @param int|null $per_page
     * @return object|Fractal|Collection                   output result
     */
    protected function execute(array $columns = ['*'], $single_object = false, int $per_page = null) {
        // Check Cache
        if ($this->cache_key) {
        ...
        if ($per_page) {
            ...
            // get|first
            $method = $single_object ? 'first' : 'get';
